...
// way to loadernstrate Rebound animations on the web for a
...
// Custom SETTINGS for each loader in related index.html
...
var loader = {
  ...
  initRebound: function initRebound() {
    ...
    var settings = loader.settings.rebound;

    // Create a SpringSystem.
    var springSystem = new rebound.SpringSystem();

    // Add a spring to the system.
    loader.spring = springSystem.createSpring(settings.tension, settings.friction);
  },
  ...
  initSpinner: function initSpinner() {
    ...
    var settings = loader.settings.spinner;

    // Instantiate Spinner.
    loader.spinner = new Spinner(settings);
  },
  ...
  Init: function init() {
    ...
    loader.spinner.init(loader.spring, spinnerTypeAutoSpin);
  ...
  loadSomething: function loadSomething() {
    ...
    oReq.addEventListener('progress', function (oEvent) {
      if (oEvent.lengthComputable) {
        ...
        loader.spring.setEndValue(percent * 0.01);
      }
    });

    oReq.addEventListener('load', function (e) {
      // Complete the loading animation.
      loader.spinner.setComplete();
    });
...
$Flinger.Loader = loader;
$Flinger.Loader.Finish = function(){
    loader.spinner.setComplete();
}